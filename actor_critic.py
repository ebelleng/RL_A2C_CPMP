...
class ActorCritic:
    ...
    def train_a2c(self,states,actions,advantages,rewards,verbose=False):
        # Se entrena actor
        ...
        # print('Termino entrenamiento actor')
    ...
    def _solve(self, Layout, solver,train=True,n_pasos=1):
        ...
        while not done:
            ...
            if done:
                #recompensa final
                reward = -len(solver(copy.deepcopy(layout))) - 1
            else:
                reward = (BP_i - BP_i_) - 1
        ...
        actions_greedy = solver(copy.deepcopy(layout))
        
        for action in actions_greedy:
            new_state, _, done = self.env.step(layout, action)
            actions.append(action)
        ...
        if train:
            self.train_a2c(states,actions,advantages,rewards)
