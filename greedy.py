...
# class Greedy:
#     def SF_move_(self, layout, pos=0):
#         s_o = None
#         s_d = None
#         min_dif = 10000
#         actions = []
...
#     def SF_move(self, layout, pos=0):
#         s, n_act = self.SF_move_(layout, pos)
...
#     def select_dismantling_stack(self, layout):
#         best_ev = 0; s_o=-1
#         for i in range(len(layout.stacks)):
#             if layout.is_sorted_stack(i) and self.capacity(layout,i)<3*len(layout.stacks[i]): continue
...
#     def select_dismantling_stacks(self, layout):
#         best_ev = 0
#         actions = []
#         for i in range(len(layout.stacks)):
#             prom = sum(layout.stacks[i]) / len(layout.stacks[i]) 
...
#     def SD_move(self, layout, pos=0,  stack=-1):
#         if len(layout.stacks[stack]) == 0: return False,0

#         if stack==-1:
#             actions = self.select_dismantling_stacks(layout)
#         else:
#             actions = [(0,stack)]
...
#     def capacity(self, layout, s_o, s_d=None):
#         capacity = 0
#         for i in range(len(layout.stacks)):
#             if i==s_o or i==s_d: continue
#             capacity += layout.H - len(layout.stacks[i])
...
#     #busca contenedor con gvalue entre c y c_ub
#     #si se encuentra mal ubicado lo intenta colocar en s_d
#     def force_move2(self, layout, c, c_ub, s_d):
...
#     #intenta mover el contenedor s_o[pos] al stack s_d
#     # def force_move(self, layout, s_o, pos, s_d): #pixie
#     #     c=layout.stacks[s_o][pos]
#     #     #if capacity(layout,s_o,s_d) < -pos-1 : return c, "failed"
#     #     while pos<-1:
#     #         s_tmp, _ = Layout.select_destination_stack(layout, s_o, black_list=[s_d])
...
#     def search_highest(self, layout, c, ub, s_d):
#         ret = None
#         for pos in range(-1,-10,-1):
#             max_c = 0
#             for s in range(len(layout.stacks)):
...
#                 if ss[pos]>=c and ss[pos]<=ub and ss[pos]>max_c: 
#                     #if pos >= -1 or ss[pos]==c:
#                     if layout.sorted_elements[s] <= len(layout.stacks[s])+pos and self.capacity(layout,s,s_d) >= -pos-1:
#                         max_c=ss[pos]
#                         ret= (s,pos)
#             if ret != None: return ret
#         return None
...
#     def iter(self, layout):
#         if not self.SF_move(layout)[0]:
#             self.SD_move(layout)
#         if layout.steps>1000: return 1000


#     def greedy_solve(self, layout):
#         while layout.unsorted_stacks>0:
#             self.iter(layout)
#         return layout.moves

#     def lazy_greedy(self, layout):
#         while layout.unsorted_stacks>0:
#             success, __ = self.SF_move(layout)
#             if not success: return

#     def simulation(self, layout, actions, min_action=0):
#         nb_actions=0
#         for a in actions:
#             a -= min_action
#             if layout.unsorted_stacks==0: break
            
#             ret, nb_actions = self.SF_move(layout,a)
#             if ret==False and nb_actions>0: return False
            
#             if not ret and nb_actions==0:
#                 ret, nb_actions = self.SD_move(layout,a)
#                 if ret ==False: return False
        
#         self.greedy_solve(layout)
#         return layout.steps, nb_actions